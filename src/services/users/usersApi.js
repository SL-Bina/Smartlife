...
export const usersAPI = {
	...
	getRoleById: async (roleId) => {
		try {
			const res = await api.get(`/module/roles/${roleId}`);
			return res.data;
		} catch (error) {
			throw error.response?.data || error.message;
		}
	...
	getComplexesByMtk: async (mtkIds = []) => {
		try {
			const params = {};
			if (Array.isArray(mtkIds) && mtkIds.length > 0) {
				params.mtk_ids = mtkIds;
			}
			const res = await api.get("/search/module/complex", { params });
			return res.data;
		} catch (error) {
			throw error.response?.data || error.message;
		}
