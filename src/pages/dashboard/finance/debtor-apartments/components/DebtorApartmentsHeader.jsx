...
export function DebtorApartmentsHeader() {
  const { t } = useTranslation();
  const gradientStyle = { background: `linear-gradient(to right, rgba(234,88,12,0.9), rgba(234,88,12,0.7), rgba(234,88,12,0.9))` };
  const borderColor = "rgba(234,88,12,0.3)";
      ...
      <div className="relative flex items-center gap-2 sm:gap-3 md:gap-4">
        <div className="flex-shrink-0">
          <div
            ...
            style={{ backgroundColor: "rgba(234,88,12,0.2)" }}
          >
            <HomeModernIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 text-white" />
          </div>
        </div>
        <div className="flex-1 min-w-0">
          <Typography variant="h4" className="text-white font-bold mb-0.5 sm:mb-1 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-base sm:text-lg md:text-xl lg:text-2xl">
            <span className="whitespace-nowrap">{t("debtorApartments.pageTitle")}</span>
          </Typography>
          <Typography className="text-white/90 dark:text-gray-300 text-xs sm:text-sm font-medium truncate sm:whitespace-normal">
            {t("debtorApartments.pageSubtitle") || "Borclu mənzillər siyahısı, bax / ödə"}
          </Typography>
        </div>
      </div>
