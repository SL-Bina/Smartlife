...
export function useExpensesData(filters, page, refreshKey = 0, sortConfig = { key: null, direction: "asc" }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [allExpensesResult, totalExpensesResult, bankTotalResult, cashTotalResult] = await Promise.all([
          fetchExpenses(filters, 1, 10000),
          fetchTotalExpenses(filters),
          fetchTotalExpensesByMethod(filters, "Bank"),
          fetchTotalExpensesByMethod(filters, "Nağd"),
        ]);

        let sortedData = allExpensesResult.data || [];
