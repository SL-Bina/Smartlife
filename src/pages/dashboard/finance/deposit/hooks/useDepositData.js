...
export function useDepositData(filters, page, refreshKey = 0, sortConfig = { key: null, direction: "asc" }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [allDepositsResult, totalDepositResult] = await Promise.all([
          fetchDeposits(filters, 1, 10000),
          fetchTotalDeposit(filters),
        ]);

        let sortedData = allDepositsResult.data || [];
        ...
        if (sortConfig.key) {
          sortedData = [...sortedData].sort((a, b) => {
            ...
            // Handle date values
            else if (sortConfig.key === "depositDate") {
