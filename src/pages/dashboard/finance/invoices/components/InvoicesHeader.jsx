...
export function InvoicesHeader() {
  ...
  const gradientStyle = {
    background: `linear-gradient(to right, rgba(34, 197, 94, 0.9), rgba(34, 197, 94, 0.7), rgba(34, 197, 94, 0.9))`,
  };

  const borderColor = "rgba(34, 197, 94, 0.3)";
    ...
    >
      ...
      <div className="relative flex items-center gap-2 sm:gap-3 md:gap-4">
        <div className="flex-shrink-0">
          ...
          >
            <DocumentTextIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 text-white" />
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            <span className="whitespace-nowrap">{t("invoices.pageTitle") || "Faktura İdarəetməsi"}</span>
          </Typography>
          <Typography className="text-white/90 dark:text-gray-300 text-xs sm:text-sm font-medium truncate sm:whitespace-normal">
            {t("invoices.pageSubtitle") || "Faktura siyahısı, yarat / redaktə et / sil / bax"}
          </Typography>
        </div>
      </div>
