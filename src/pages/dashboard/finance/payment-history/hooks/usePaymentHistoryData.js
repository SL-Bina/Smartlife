...
export function usePaymentHistoryData(filters, page, refreshKey = 0, sortConfig = { key: null, direction: "asc" }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [allPaymentsResult, totalAmountResult] = await Promise.all([
          fetchPaymentHistory(filters, 1, 10000),
          fetchTotalAmount(filters),
        ]);

        let sortedData = allPaymentsResult.data || [];
