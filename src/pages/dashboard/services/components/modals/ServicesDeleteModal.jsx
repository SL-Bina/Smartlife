  ...
  return (
    ...
    >
      <DialogHeader className="dark:bg-gray-800 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-4">
        <div className="flex items-center gap-2">
          ...
          <Typography variant="h5" className="font-bold">
            {t("services.delete.title") || "Servis Sil"}
          </Typography>
        </div>
      </DialogHeader>
      <DialogBody divider className="space-y-4 dark:bg-gray-800 dark:border-gray-700 py-4">
        <Typography variant="small" color="blue-gray" className="dark:text-gray-300">
          {t("services.delete.message") || "Bu servisi silmək istədiyinizə əminsiniz?"}
          <br />
          <strong className="text-gray-900 dark:text-white">{service.name}</strong> (ID: {service.id})
        </Typography>
        <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          <Typography variant="small" className="text-red-700 dark:text-red-300">
            {t("services.delete.warning") || "Bu əməliyyat geri alına bilməz."}
          </Typography>
        </div>
      </DialogBody>
      <DialogFooter className="flex justify-end gap-2 dark:bg-gray-800 dark:border-gray-700 pt-4">
        ...
        >
          {t("services.cancel") || t("buttons.cancel")}
        </Button>
        <Button
          color="red"
          onClick={onConfirm}
        ...
        >
          {deleting ? (
            <span className="flex items-center gap-2">
              ...
              {t("services.deleting") || "Silinir..."}
            </span>
          ) : (
            t("services.actions.delete") || t("buttons.delete")
          )}
        </Button>
      </DialogFooter>
    </Dialog>
