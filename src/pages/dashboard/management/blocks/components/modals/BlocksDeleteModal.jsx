...
export function BlocksDeleteModal({ open, onClose, item, onConfirm }) {
  ...
  return (
    <Dialog open={!!open} handler={onClose} size="sm" className="dark:bg-gray-800">
      ...
      <DialogBody className="dark:text-gray-200">
        {!item ? (
          <Typography className="text-sm text-blue-gray-500 dark:text-gray-300">Seçilən blok yoxdur</Typography>
        ) : (
          <Typography>
            <b>{item.name}</b> adlı blok silinəcək. Davam edək?
          </Typography>
        )}
      </DialogBody>
