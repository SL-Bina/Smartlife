...
const Bread404 = () => {
  return (
    <div className="min-h-[600px] mx-auto w-auto max-w-[460px] flex items-center justify-center">
      <svg
...
export function NotFound() {
  ...
  React.useEffect(() => {
    ...
    document.body.setAttribute('data-404-page', 'true');
    
    // Hide all dashboard layout elements directly
    const sidebar = document.querySelector('[class*="sidenav"]');
    const navbar = document.querySelector('[class*="navbar"]');
    const configurator = document.querySelector('[class*="configurator"]');
    const footer = document.querySelector('footer');
    const aiChat = document.querySelector('[class*="ai-chat"]');
    ...
    return () => {
      document.body.removeAttribute('data-404-page');
      if (sidebar) sidebar.style.display = '';
      if (navbar) navbar.style.display = '';
      if (configurator) configurator.style.display = '';
      if (footer) footer.style.display = '';
      if (aiChat) aiChat.style.display = '';
  ...
  return (
    <div 
      data-404-page="true"
      className="fixed inset-0 w-full h-full flex flex-col items-center justify-center bg-white dark:bg-gray-900 overflow-hidden"
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
    ...
    >
      ...
      <div className="relative z-10 text-center px-4">
        <Typography
          variant="h3"
          className="text-gray-800 dark:text-white text-2xl sm:text-3xl font-bold mb-4"
        >
          {t("404.title") || "Səhifə tapılmadı"}
        </Typography>
        <Typography
          variant="paragraph"
          className="text-gray-600 dark:text-gray-400 font-medium text-base sm:text-lg mb-6"
        >
          {t("404.message") ||
            "Təəssüf ki, bu səhifə mövcud deyil. Ana səhifəyə qayıda bilərsiniz."}
        </Typography>
        <Button
          onClick={handleGoHome}
          className="bg-cyan-500 hover:bg-cyan-600 text-white px-8 py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2 mx-auto"
          size="lg"
        >
          <HomeIcon className="h-5 w-5" />
          {t("404.goHome") || "Ana Səhifəyə Qayıt"}
        </Button>
      </div>
    </div>
  );
