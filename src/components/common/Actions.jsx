...
export function Actions({
  ...
  useEffect(() => {
    ...
    const readFirstId = (value) => {
      if (Array.isArray(value)) return value[0] || null;
      return value || null;
    };

    const idByType = {
      mtk: readFirstId(search.mtkId ?? search["mtk_ids[]"]),
      complex: readFirstId(search.complexId ?? search["complex_ids[]"]),
      building: readFirstId(search.buildingId ?? search["building_ids[]"]),
      block: readFirstId(search.blockId ?? search["block_ids[]"]),
      property: readFirstId(search.propertyId ?? search["property_ids[]"]),
    };

    const labelStateKeys = {
      mtk: "mtk",
      complex: "complex",
      building: "building",
      block: "block",
    ...
    const toResolve = Object.entries(idByType).filter(([type, id]) => id && !selectedLabels[labelStateKeys[type]]);
    if (toResolve.length === 0) return;

    let active = true;

    Promise.all(
      toResolve.map(async ([type, id]) => {
        try {
          const endpoint = FILTER_CONFIG[type]?.endpoint;
          if (!endpoint) return [type, null];

          const params = { [selfParamKey[type]]: [id], per_page: 1, page: 1 };
          const response = await api.get(endpoint, { params });
          const item = parseFirstItem(response);
          const label = item?.name || item?.apartment_number || null;
          return [type, label];
        } catch {
          return [type, null];
        }
      ...
      setSelectedLabels((prev) => {
        ...
        entries.forEach(([type, label]) => {
          if (!label) return;
          next[labelStateKeys[type]] = label;
        });
        return next;
      });
    });
  ...
  const clearSelectedLabelCascade = (normalizedKey) => {
    setSelectedLabels((prev) => {
      const next = { ...prev };
  ...
  const handleRemoveActiveFilter = (key) => {
    ...
    clearSelectedLabelCascade(normalizedKey);
    onRemoveFilter?.(normalizedKey);
  };
  ...
  const activeFilters = useMemo(() => {
    ...
    const hierarchyLabelByKey = {
      complexId: selectedLabels.complex,
      buildingId: selectedLabels.building,
      blockId: selectedLabels.block,
      propertyId: selectedLabels.property,
      "complex_ids[]": selectedLabels.complex,
      "building_ids[]": selectedLabels.building,
      "block_ids[]": selectedLabels.block,
      "property_ids[]": selectedLabels.property,
    };
    ...
    const toDisplayValue = (key, value) => {
      const hierarchyLabel = hierarchyLabelByKey[key];
    ...
    return Object.entries(search)
      ...
      .map(([key, value]) => ({
        ...
        order: keyOrder[key] ?? 999,
      }))
      .sort((a, b) => a.order - b.order || a.label.localeCompare(b.label));
  }, [search, customFilterLabels, selectedLabels]);
            ...
            <div className="flex flex-wrap items-center gap-2">
              {activeFilters.map((filter) => (
                <Chip
                  ...
                  onClose={() => handleRemoveActiveFilter(filter.key)}
